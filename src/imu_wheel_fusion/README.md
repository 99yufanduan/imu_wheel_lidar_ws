...
# 时间同步方式
- 使用tf自动插值
    比如来了一帧imu数据，转化成imu tf，然后来了一帧wheel数据，转化成wheel tf，filter_time 通常选择为最新收到的传感器数据的时间戳,然后tf线性插值。robot_pose_ekf 包就是使用这种方式
    - 优点
        简单，不需要额外的计算
    - 缺点
        插值假设传感器数据在短时间内是线性变化的，对非线性运动（如急转弯）会引入误差。
    适合传感器数据在短时间内是匀速运动
- message_filters的插值方式
    更适合传感器之间频率稳定，比如有硬件同步，或者一个高频一个低频，可以较容易的进行时间同步
