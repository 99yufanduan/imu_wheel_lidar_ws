...
const std::string kMapPath =
    "/home/dyf/project/project_data/rosbag_0827_imu_wheel_vanjee_南风楼/map.pcd";
...
private:
  rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr
      pose_sub_; // 接收imu和wheel融合的pose
  rclcpp::Subscription<sensor_msgs::msg::PointCloud2>::SharedPtr raw_pc_sub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr
      pose_pub_; // 发布ndt match pose
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr
      ndt_pc_pub_; // 发布匹配后坐标转换的点云
  rclcpp::Subscription<geometry_msgs::msg::PoseStamped>::SharedPtr
      manual_init_pose_subscriber_;
  void poseCallback(
      const geometry_msgs::msg::PoseWithCovarianceStamped::UniquePtr pose_in);
  void pcCallback(const sensor_msgs::msg::PointCloud2::UniquePtr pc_ros_in);
  void manual_init_pose_callback(
      const geometry_msgs::msg::PoseStamped::SharedPtr msg);
  std::unique_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster_;
  std::shared_ptr<NormalDistributionsTransform> ndt_ptr_;
  std::mutex *ndt_ptr_mutex_;
  std::deque<geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr>
      imu_wheel_pose_msg_ptr_queue_;
  geometry_msgs::msg::PoseStamped manual_init_pose_;
  bool manual_pose_flag_ = 0;

public:
  NdtMatcher()
      : Node("ndt_matcher"),
        ndt_ptr_(
            new pclomp::MultiGridNormalDistributionsTransform<pcl::PointXYZ,
                                                              pcl::PointXYZ>()),
        ndt_ptr_mutex_(new std::mutex()) {
    pose_sub_ = this->create_subscription<
        geometry_msgs::msg::PoseWithCovarianceStamped>(
        "/ekf_pose_with_covariance", 5,
        std::bind(&NdtMatcher::poseCallback, this, _1));
    raw_pc_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        "/lidar_points", 5, std::bind(&NdtMatcher::pcCallback, this, _1));
    pose_pub_ =
    ...
    manual_init_pose_subscriber_ =
        this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "goal_pose", 2,
            std::bind(&NdtMatcher::manual_init_pose_callback, this,
                      std::placeholders::_1));
    ...
    ndt_params.resolution = 1.0;
    ndt_params.max_iterations = 70;
...
void NdtMatcher::manual_init_pose_callback(
    const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
  manual_pose_flag_ = true;
  manual_init_pose_ = *msg;
 ...
 * @date 2024/08/28
 */
void NdtMatcher::poseCallback(
    const geometry_msgs::msg::PoseWithCovarianceStamped::UniquePtr pose_in) {
  imu_wheel_pose_msg_ptr_queue_.push_back(
...
/**
 * @brief 接收点云数据,在imu和wheel估计的pose
 ...
 * @param[out] ndt_pose
 *
 * @todo imu_wheel state queue 做插值 , 2024/08/26
 * 插值有点问题，暂时先不用，当插入的值位姿偏差过大会assert
 * fail，并且不确定点云的时间戳在imu时间序列内
 *
 * @author y.f.duan@outlook.com
 * @date 2024/08/20
 */
void NdtMatcher::pcCallback(
    const sensor_msgs::msg::PointCloud2::UniquePtr pc_ros_in) {
  /**** 点云下采样并输入到NDT ****/
  pcl::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> point_cloud(
  ...
  voxel_filter.setLeafSize(0.2f, 0.2f, 0.2f); // 设置体素的尺寸 (5cm 的分辨率)
  // 执行滤波
  ...
  /**** 队列中找到对齐的pose，以此作为初始pose做ndt match ****/
  // TODO 2024/08/26 插值有点问题，暂时先不用，当插入的值位姿偏差过大会assert
  // fail，并且不确定点云的时间戳在imu时间序列内 if (pc_ros_in)
  ...
  auto latest_imu_wheel_pose_time =
      rclcpp::Time(imu_wheel_pose_msg_ptr_queue_.back()->header.stamp)
          .nanoseconds();
  auto oldest_imu_wheel_pose_time =
      rclcpp::Time(imu_wheel_pose_msg_ptr_queue_.front()->header.stamp)
          .nanoseconds();
  if (!imu_wheel_pose_msg_ptr_queue_.size()) {
    current_pose = previou_ndt_pose;
    std::cout << "imu_wheel_pose_queue is empty,use previou ndt pose "
              << std::endl;
  } else {
    if (current_pc_time > latest_imu_wheel_pose_time) {
      current_pose =
          pose_to_matrix4f(imu_wheel_pose_msg_ptr_queue_.back()->pose.pose);
      imu_wheel_pose_msg_ptr_queue_.clear();
      ...
      for (const auto &imu_wheel_pose : imu_wheel_pose_msg_ptr_queue_) {
        if (current_pc_time >
            rclcpp::Time(imu_wheel_pose->header.stamp).nanoseconds()) {
          imu_wheel_pose_msg_ptr_queue_.pop_front();
        } else {
          current_pose = pose_to_matrix4f(
              imu_wheel_pose_msg_ptr_queue_.front()->pose.pose);
        }
      }
    }
  }
  ...
  if (manual_pose_flag_) {
    manual_pose_flag_ = false;
    ndt_ptr_->align(*output_cloud, pose_to_matrix4f(manual_init_pose_.pose));
  } else {
    ndt_ptr_->align(*output_cloud, current_pose);
  }
  ...
  /**** publish pose  ****/
  const geometry_msgs::msg::Pose result_pose_msg =
      matrix4f_to_pose(ndt_result.pose);
...
void NdtMatcher::publish_pose(const rclcpp::Time &sensor_ros_time,
                              const geometry_msgs::msg::Pose &result_pose_msg) {
...
void NdtMatcher::update_map(
    const std::vector<ndt_msgs::msg::PointCloudMapCellWithID> &maps_to_add,
    const std::vector<std::string> &map_ids_to_remove) {
  RCLCPP_INFO(this->get_logger(), "Update map (Add: %lu, Remove: %lu)",
              maps_to_add.size(), map_ids_to_remove.size());
  ...
  for (const auto &map_to_add : maps_to_add) {
    pcl::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> map_points_ptr(
        new pcl::PointCloud<pcl::PointXYZ>);
  ...
  const double exe_time = std::chrono::duration_cast<std::chrono::microseconds>(
                              exe_end_time - exe_start_time)
                              .count() /
                          1000.0;
  RCLCPP_INFO(this->get_logger(),
              "Time duration for creating new ndt_ptr: %lf [ms]", exe_time);

  // swap
  (*ndt_ptr_mutex_).lock();
  // ToDo (kminoda): Here negligible NDT copy occurs during the new map loading
  // phase, which should ideally be avoided. But I will leave this for now since
  // I cannot come up with a solution other than using pointer of pointer.
  *ndt_ptr_ = backup_ndt;
...
int NdtMatcher::mapLoad(const std::string &map_path) {
  ...
  sor.setLeafSize(0.2f, 0.2f, 0.2f); // 设置体素的尺寸 (这里是 5cm 的分辨率)
